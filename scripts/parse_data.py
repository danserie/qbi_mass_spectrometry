...
#    pickle.dump(samples, f)
  ...
  for i, p in enumerate(total_pairs):
    if len(to_be_saved) >= 400:
      with open('/hd/data/massiveKB%d.pkl' % j, 'wb') as f:
        pickle.dump(to_be_saved, f)
      with open('/hd/data/massiveKB_raw%d.pkl' % j, 'wb') as f:
        pickle.dump(raw_saved, f)
      to_be_saved = {}
      raw_saved = {}
      j += 1
    ...
    metadata[i] = ['/hd/data/massiveKB%d.pkl' % j, p[0].shape[0]]
  ...
  with open('/hd/data/massiveKB_meta.pkl', 'wb') as f:
